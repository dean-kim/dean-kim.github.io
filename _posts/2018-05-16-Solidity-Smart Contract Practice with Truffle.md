...
TestNoshow 파일을 열고 다음과 같이 작성합니다.
...
<tt style="color: #FF0000">`Solidity 0.4.4`</tt> 버전을 사용함을 선언합니다.
 
<tt style="color: #FF0000">`import "truffle/Assert.sol";`</tt>는 테스트에서 사용할 <tt style="color: #FF0000">`assertion`</tt>을 import하는 구문입니다.

<tt style="color: #FF0000">`import "truffle/DeployedAddresses.sol";`</tt>는 배포된 contract address를 가져옵니다.

<tt style="color: #FF0000">`import "../contracts/NoShow.sol";`</tt>는 테스트할 contract 코드를 가져옵니다.

그리고 test를 다음의 명령어로 실행합니다.
...
Compiling ./contracts/NoShow.sol...
...
0 passing
~~~~

통과된 테스트들이 0이라고 나옵니다. 아직 테스트와 계약을 작성하지 않았으니 나오는 당연한 결과입니다.

#### check owner

이 contract를 배포한 사람이 owner인지 확인하는 테스트를 작성합니다.
...
테스트 코드에서 contract를 생성하고, 생성자가 owner인지를 <tt style="color: #FF0000">`Assert`</tt>의 <tt style="color: #FF0000">`equal`</tt>을 통해 검증합니다.
...
그러면 다음과 같은 메세지를 확인할 수 있습니다.

~~~~
...
TypeError: Member "owner" not found or not visible after argument-dependent lookup in contract NoShow
...
~~~~

작성한 contract에 owner가 없다는 오류메세지 입니다.

여기서 우리는 contract에 owner가 필요하다는 사실을 알게 되었습니다.

그럼 contract code에 owner를 추가해줍시다.
...
~~~~
TestNoshow
    1) testSettingAnOwnerDuringCreation

    Events emitted during test:
    ---------------------------

    TestEvent(result: <indexed>, message: An owner is different than a deployer)
  ...
  1) TestNoshow testSettingAnOwnerDuringCreation:
     Error: An owner is different than a deployer
~~~~

owner가 deployer와 다르다고 합니다.

그러면 contract code에서 public으로 선언한 address 타입의 변수 owner에 deployer를 할당하도록 작성합니다.
...
이제 작성한 테스트를 통과할 수 있는 조건이 갖추어 졌습니다.
